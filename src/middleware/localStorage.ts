...
import { LANG_STATE, LOGIN_STATE, RANGE_STATE } from "../constants/local-storage";
...
> = (store) => (next) => (action) => {  
  ...
  if (selectLangCode.match(action)) {
    ...
    const langState = { langCode: langCode, langName: langName  }
  ...
  if (selectRangeCode.match(action)) {
    ...
    const rangeState = { rangeCode: rangeCode, rangeName: rangeName }
